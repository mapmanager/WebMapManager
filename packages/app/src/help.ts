...
export const openHelpMenu = async () => {
  // Create a blocker element that lets the user select the element they need
  // help with.
  // All elements with a data-intro attribute will show as targets for the help
  // menu.
  const blocker = document.createElement("div");
  blocker.className =
    "fixed top-0 left-0 w-full h-full z-[100000] bg-black/50 cursor-pointer text-white text-2xl flex items-center justify-center";
  blocker.textContent = "Click on the item you need help with";
  document.body.appendChild(blocker);

  const borderBox = document.createElement("div");
  borderBox.className =
    "fixed pointer-events-none border-2 border-yellow-300 rounded-md block";
  blocker.appendChild(borderBox);

  const potentialTargets = document.createElement("div");
  potentialTargets.className =
    "fixed top-0 left-0 w-full h-full pointer-events-none opacity-50";
